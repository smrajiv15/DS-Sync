...
struct time_reply slave2_offset;
char ip_address[256];
int cached = 0;
...
void time_sync_set(int socket_c, struct client_request *req) {
	...
	memcpy(&slave2_offset, &(req->off), sizeof(struct time_reply));
...
void contact_server(struct client_request *req, union server_response *resp) {
        ...
        if(connect(socket_c,(struct sockaddr *)&server,sizeof(struct sockaddr_in)) < 0) {
                error("Connecting Server Failed\n");
        }
...
void set_time(struct client_request *req) {
	...
	pthread_mutex_lock(&off_lock);
        if(slave2_offset.s_sign == '+') {
                req->off.secs = tm_p->tm_sec + slave2_offset.secs;
        } else {
                req->off.secs = tm_p->tm_sec - slave2_offset.secs;
        }

        if(slave2_offset.m_sign == '+') {
                req->off.msecs = (tv.tv_usec / 1000) + slave2_offset.msecs;
        } else {
		if((tv.tv_usec / 1000) < slave2_offset.msecs) {
                	req->off.msecs = ((1000 + (tv.tv_usec / 1000)) - slave2_offset.msecs);
                	req->off.secs--;
		} else {
                	req->off.msecs = (tv.tv_usec / 1000) - slave2_offset.msecs;
		}
